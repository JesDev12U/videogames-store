...
Promise.all([
  ...
  .then((json) => {
    ...
    for (let platform in playstationObject) {
      let $cloneSlide;
      let $clonePlatform = document.importNode($temSwiperContainer, true);
      for (let videogame in playstationObject[platform]) {
        $cloneSlide = document.importNode($temSwiperSlide, true);
        let $videogame = $cloneSlide.querySelector(".videogame");
        $videogame.dataset.front = `./assets/img/playstation/${platform.toLowerCase()}/${videogame
          .toLowerCase()
          .trim()}/front.jpg`;
        $videogame.dataset.back = `./assets/img/playstation/${platform.toLowerCase()}/${videogame
          .toLowerCase()
          .trim()}/back.jpg`;
